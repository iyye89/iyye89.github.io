...
BIT.Core.ObjectStorage.prototype = {
    ...
    setItem: function(a, b) {
        if (undefined === this.defaultData[a]) {
            console.warn("nie mozna zapisac tej opcji, pole niedostepne");
            return false;
        }
        this.data[a] = b;
        this.save();
    ...
    pushToItem: function(a, c) {
        ...
        if (false === $u.isArray(b)) {
            console.warn("to pole nie jest tablica");
            return false;
        }
        this.data[a].push(c);
        this.save();
...
BIT.Game.GameStorage = new BIT.Core.ObjectStorage("__animory", {
    music: false,
    sound2: true,
    allPoints: 0,
    unlockPieces: []
