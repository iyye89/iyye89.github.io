function boosterLoaded(a, b, c, d) {
    adSense = a, community = b, analytics = c, moregames = d, loadPreAssets()
}
...
function initStartScreen() {
    gameState = "start", analytics.menu(), userInput.removeHitArea("moreGames"), 1 == audioType && (musicTween && musicTween.kill(), musicTween = TweenLite.to(music, 1, {
        volume: .2,
...
function initGameEnd() {
    ...
    }, panel.startTween1(), community.submitScore({
        score: panel.oScoreData.dist,
        callback: function() {
            adSense.showAdvertising()
        }
    }), analytics.score(panel.oScoreData.dist), previousTime = (new Date).getTime(), updateGameEndEvent()
}
...
var panel, hud, background, cat, gauge, offsetX, aGElements, gameTouchState, aimX, aimY, targAimX, targAimY, flyX, flyY, vx, vy, gravity = 500,
    ...
    musicTween, adSense, community, analytics, moregames;