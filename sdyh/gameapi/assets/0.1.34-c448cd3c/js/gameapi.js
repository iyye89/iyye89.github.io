...
fg_api.prototype.adsModule = function() {
	...
	module.prototype.rewardedAd = function(callback, force) {
		...
		M.origVastUrl = self.config.ads.vast_url;

		M.rewVastUrl = 'https://ads.aerserv.com/as/?plc=1017787&cb=[CACHEBUSTER]&url=[URL]&ip=&make=&model=&os=&osv=&type=[DEVICECATEGORY]&lat=[LAT]&long=[LON]&locationsource=2&ua=[UA]&vpw=320&vph=480&vpaid=1&coppa=&age=&yob=&gender=';

		self.config.ads.vast_url = M.rewVastUrl;
		M.forceAd(function () {
			self.config.ads.vast_url = M.origVastUrl;
		...
		return new Promise(function(resolve, reject) {
			...
			if (self.config.ads.vast_url) {
				...
				self.geo.getLatLon().then(function(latlong) {
					...
					vast_url = vast_url.replace(/\[URL\]/, encodeURIComponent(document.location.href));
	...
	module.prototype.showAdModal = function(options, contentDiv) {
		...
		} else if ('scrollIntoView' in self.bodyElement) {
			self.bodyElement.scrollIntoView();
		}
...
fg_api.prototype.firebaseModule = function() {
	...
	module.prototype.initializeFirebase = function() {
		...
		M.promise = new Promise(function(resolve, reject) {
			...
			// faZepto.getScript('https://www.gstatic.com/firebasejs/3.4.0/firebase.js', function() {
			// 	// Initialize Firebase
			// 	var config = {
